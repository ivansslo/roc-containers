...
REPO_SLUG="ivansslo/roc-containers"
...
echo -e "${BOLD}${CYAN}"
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║          roc-containers · Safe Push (via GitHub CLI)       ║"
echo "  ╚══════════════════════════════════════════════════════╝"
echo -e "${RESET}"
...
if [ ! -d .git ]; then
  ...
  err "Jalankan push.sh dari dalam folder repo roc-containers."
  exit 1
fi
...
if [ -n "$(git status --porcelain)" ]; then
  ...
  git status -s
  echo ""
  read -r -p "  Commit semua perubahan ini dulu? [y/N]: " yn || yn=""
  if [ "${yn,,}" = "y" ]; then
    read -r -p "  Pesan commit: " msg || msg=""
    [ -z "$msg" ] && msg="Update roc-containers"
    git add -A
    git commit -m "$msg"
