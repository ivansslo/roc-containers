...
# ─────────────────────────────────────────────────────────────────
#  isdocker · Jellyfin Media Server
...
if [ -n "$1" ]; then
  ...
  udocker_run --entrypoint "bash -c" -p "${PORT}:8096" \
    ...
    $_LIBNETSTUB_VOL \
    -v "$DATA_DIR/config:/config" -v "$DATA_DIR/cache:/cache" $MEDIA_DIR_CONFIG \
    "$CONTAINER_NAME" ". /.libnetstub/libnetstub.sh; $cmd"
else
  udocker_run --entrypoint "bash -c" -p "${PORT}:8096" \
    -e _PORT="$PORT" -e DOTNET_GCHeapHardLimit="1C0000000" \
    $_LIBNETSTUB_VOL \
    -v "$DATA_DIR/config:/config" -v "$DATA_DIR/cache:/cache" $MEDIA_DIR_CONFIG \
    "$CONTAINER_NAME" '
      ...
      xmlstarlet ed --inplace \
          -u "//InternalHttpPort" -v "$_PORT" \
          -u "//PublicHttpPort"   -v "$_PORT" \
          /config/config/network.xml &>/dev/null
