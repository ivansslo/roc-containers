...
if [ -n "$1" ]; then
  ...
  udocker_run --entrypoint "bash -c" -p "${PORT}:8123" \
    ...
    "$CONTAINER_NAME" '
      ...
      command -v start-stop-daemon &>/dev/null || apt install -y --no-install-recommends s-bb
      command -v yq &>/dev/null || apt install -y --no-install-recommends yq
      ...
      _PIDFILE="$(mktemp)"
      start-stop-daemon -mp "$_PIDFILE" --stdout /var/log/home-assistant/stdout.log --stderr /var/log/home-assistant/stderr.log \
          -bSx "$(command -v bash)" -- -c "exec $(command -v python3) -m homeassistant --config /config --verbose" && \
